...
def indentation_fit(path):
    ...
    def fit_func_FungModel(x, E0, b):
        '''Fung model for hyperelastic materials, from Lin, David C., et al., 
        Biomechanics and modeling in mechanobiology 8.5 (2009): 345-358.'''
        nu = 0.5
        x = np.abs(x)
        a = np.sqrt(R_tip * x)
        B = (20*E0) / (9*np.pi*(1-nu**2))
        f = B*np.pi * \
            (((a**5 - 15*R_tip*a**4 + 75*(R_tip**2)*a**3) / (5*R_tip*a**2 - 50*(R_tip**2)*a + 125*R_tip**3))) \
            * np.exp(b * (a**3 - 15*R_tip*a**2) / (25*(R_tip**2) * a - 125*R_tip**3))
        return f

    # Applying the fit to the indentation portion
    fitting_models = [fit_func_hertz, fit_func_modifhertz_nb, fit_func_modifhertz_b,
                      fit_func_garcia_conicalTip, fit_func_FungModel]
    ...
    try:
        ...
        # (2009): 345-358
        R2 = 1 - (norm(fit_force - fit) ** 2 / norm(fit_force - np.mean(fit_force)) ** 2)
        # print('R square: ', R2)
    except:
        R2 = 0
    # Young modulus from indentation energy and classic Hertz model (not used in later analyses)
    ...
    # plt.pause(0.7)
    # plt.close()
    return x_pos, y_pos, young_modulus, R2, cp_x, area_simps, E_bis
